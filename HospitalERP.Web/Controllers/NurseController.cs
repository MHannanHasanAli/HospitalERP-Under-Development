...
namespace HospitalERP.Web.Controllers
{
    public class NurseController : Controller
    {
        ...
        public NurseController(UserManager<User> userManager, RoleManager<IdentityRole> roleManager)
        {
        ...
        {
            var model = new NurseListingViewModel();

            foreach (var user in userManager.Users)
            {
                if (await userManager.IsInRoleAsync(user, "Nurse"))
                {
                    model.Nurses.Add(user);
                }
            }
        ...
        public async Task<IActionResult> Action(string Id = "0", int view = 0)
        {
            if (Id == "0")
            {
                var Createmodel = new NurseActionViewModel();
                Createmodel.Id = "0";
            ...
            var model = new NurseActionViewModel(user);
        ...
        [HttpPost]
        public async Task<IActionResult> Action(NurseActionViewModel model)
        {
            ...
            if (model.Id == "0")
            {
                ...
                var result = await userManager.CreateAsync(Nurse, model.Password);
                ...
                await userManager.AddToRoleAsync(Nurse, role.Name);
            }
            else
            {
